...
#[cfg(all(feature = "sha3", not(any(target_os = "trezoa", target_arch = "bpf"))))]
...
pub fn hashv(vals: &[&[u8]]) -> Hash {
    ...
    #[cfg(not(any(target_os = "trezoa", target_arch = "bpf")))]
    {
        ...
        #[cfg(not(feature = "sha3"))]
        {
            ...
            panic!("hashv is only available on target `trezoa` or with the `sha3` feature enabled on this crate")
        }
    }
    ...
    #[cfg(any(target_os = "trezoa", target_arch = "bpf"))]
    {
        ...
        unsafe {
            trezoa_define_syscall::definitions::sol_keccak256(
